window.onload = () => {
  ...
  let opts = {
    ...
    staticZones: [
      ...
      // Yellow
      {
        strokeStyle: "linear-gradient(to right, rgba(255,0,0,0), rgba(255,0,0,1));",
        min: 18.5,
        max: 25,
      },
      // Green
...
*/
  ...
  const addSelected = (e, i) => {
    ...
    if (selectedOption) {
      ...
      delt = document.querySelectorAll(".delt");
      delt.forEach((i) => {
        i.addEventListener("click", (e) => {
      ...
      daysForThis.forEach((j) => {
        ...
        newp.outerHTML = `<p class="${selectedOption}">${selectedOption} + ', </p>`;
